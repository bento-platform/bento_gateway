...
if [[ "$(true_values_to_1 $BENTO_BEACON_ENABLED)" == 1 ]]; then
  echo "[bento_gateway] [entrypoint] Fine-tuning nginx.conf to use Beacon"
else
  echo "[bento_gateway] [entrypoint] Fine-tuning nginx.conf to disable Beacon"
  sed -i.bak \
      '/tpl__use_beacon__start/,/tpl__use_beacon__end/d' \
      ./nginx.conf.pre
fi
